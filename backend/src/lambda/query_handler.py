...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            # Try FAISS search first for better performance
            relevant_chunks = vector_search.hybrid_search_faiss(
                query=question,
                query_embedding=question_embedding,
                book_title=book_title
            )
            print(f"FAISS search successful, found {len(relevant_chunks)} chunks")
        except Exception as e:
            print(f"FAISS search failed, falling back to original method: {e}")
            ...
            print(f"Fallback search found {len(relevant_chunks)} chunks")
